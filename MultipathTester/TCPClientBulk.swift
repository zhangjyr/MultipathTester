...
class TCPClientBulk {
    ...
    func Run() -> [String: Any] {
        ...
        var fd = findTCPFileDescriptor(expectedIPs: ["5.196.169.232"], expectedPort: 80, exclude: -1)
        if (fd < 0) {
            while (res == .timedOut && fd < 0) {
                ...
                fd = findTCPFileDescriptor(expectedIPs: ["5.196.169.232"], expectedPort: 80, exclude: -1)
            }
        }
        print("FD is \(fd)")
