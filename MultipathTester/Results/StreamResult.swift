...
class StreamResult: BaseResult, TestResult {
    ...
    static func getCollectURL() -> URL {
        return URL(string: collectURLBase + "stream/test/")!
    }
    ...
    // MARK: Initializers
    init(name: String, proto: NetProtocol, success: Bool, result: String, duration: Double, startTime: Date, waitTime: Double, wifiBytesReceived: UInt32, wifiBytesSent: UInt32, cellBytesReceived: UInt32, cellBytesSent: UInt32, multipathService: RunConfig.MultipathServiceType, upDelays: [DelayData], downDelays: [DelayData], errorMsg: String) {
